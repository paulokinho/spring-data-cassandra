...
/**
 * <b>This is the central class in the Cassandra core package.</b> It simplifies the use of CQL and helps to avoid
 * common errors. It executes core CQL workflow, leaving application code to provide CQL and extract results. This class
 * executes CQL queries or updates, initiating iteration over {@link ReactiveResultSet}s and catching
 * {@link DriverException} exceptions and translating them to the generic, more informative exception hierarchy defined
 * in the {@code org.springframework.dao} package.
 * <p>
...
public class ReactiveCqlTemplate extends ReactiveCassandraAccessor implements ReactiveCqlOperations {
	 ...
	 *
	 * @param reactiveSessionFactory the {@link ReactiveSessionFactory} to obtain {@link ReactiveSession}s from.
	 */
	...
	// -------------------------------------------------------------------------
	// Methods dealing with static CQL (com.datastax.driver.core.Statement)
	// -------------------------------------------------------------------------
	...
	public Flux<Row> queryForRows(String cql) throws DataAccessException {
		return queryForResultSet(cql).flatMap(ReactiveResultSet::rows).onErrorResumeWith(translateException());
	}
	...
	public <T> Flux<T> execute(ReactivePreparedStatementCreator psc, ReactivePreparedStatementCallback<T> action)
		...
		return createFlux(session -> psc.createPreparedStatement(session).doOnNext(this::applyStatementSettings)
	...
	public Flux<Row> queryForRows(String cql, Object... args) throws DataAccessException {
		return queryForResultSet(cql, args).flatMap(ReactiveResultSet::rows).onErrorResumeWith(translateException());
	}
	...
	public Mono<Boolean> execute(ReactivePreparedStatementCreator psc) throws DataAccessException {
		...
		return execute(psc, (session, ps) -> {

			if (logger.isDebugEnabled()) {
				logger.debug("Executing Prepared CQL Statement [{}]", ps.getQueryString());
			}

			return session.execute(ps.bind()).map(ReactiveResultSet::wasApplied);
		}).next();
	}
	...
	public Flux<Boolean> execute(String cql, Publisher<Object[]> args) throws DataAccessException {
		...
		return execute(psc, (session, ps) -> Flux.from(args).flatMap(objects -> {
			...
			if (logger.isDebugEnabled()) {
				logger.debug("Executing Parepared CQL Statement [{}]", cql);
			}
	...
	/**
	 * Create a reusable {@link Flux} given a {@link ReactiveStatementCallback}.
	 *
	 * @param callback must not be {@literal null}.
	 * @return a reusable {@link Flux} wrapping the {@link ReactiveStatementCallback}.
	 */
	public <T> Flux<T> createFlux(Statement statement, ReactiveStatementCallback<T> callback) {
		...
		return Flux.defer(() -> callback.doInStatement(session, statement)).onErrorResumeWith(translateException());
	}

	/**
	 * Create a reusable {@link Mono} given a {@link ReactiveStatementCallback}.
	 *
	 * @param callback must not be {@literal null}.
	 * @return a reusable {@link Mono} wrapping the {@link ReactiveStatementCallback }.
	 */
	public <T> Mono<T> createMono(Statement statement, ReactiveStatementCallback<T> callback) {
		...
		return Mono.defer(() -> Mono.from(callback.doInStatement(session, statement))).otherwise(translateException());
	}
	 ...
	 */
	public <T> Flux<T> createFlux(ReactiveSessionCallback<T> callback) {

		Assert.notNull(callback);

		ReactiveSession session = getSession();

		return Flux.defer(() -> callback.doInSession(session)).onErrorResumeWith(translateException());
	}
	...
	private class SimpleReactivePreparedStatementCreator implements ReactivePreparedStatementCreator {
