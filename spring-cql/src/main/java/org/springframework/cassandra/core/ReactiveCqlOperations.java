...
public interface ReactiveCqlOperations {
	...
	/**
	 * Execute a CQL data access operation, implemented as callback action working on a {@link ReactiveSession}. This allows for
	 * implementing arbitrary data access operations, within Spring's managed CQL environment: that is, converting CQL
	...
	/**
	 * Issue an statement using a {@link PreparedStatementBinder} to set bind parameters, with given CQL. Simpler
	 * than using a {@link ReactivePreparedStatementCreator} as this method will create the {@link PreparedStatement}: The
	...
	/**
	 * Issue a single CQL operation (such as an insert, update or delete statement) via a prepared statement,
