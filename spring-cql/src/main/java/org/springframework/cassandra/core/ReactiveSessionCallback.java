...
public interface ReactiveSessionCallback<T> {
	...
	/**
	 * Gets called by {@link ReactiveCqlTemplate#execute(ReactiveSessionCallback)} with an active CQL session. Does not
	 ...
	 * {@link ReactiveCqlTemplate#queryForObject(Statement, Class)} etc. A thrown RuntimeException is treated as
